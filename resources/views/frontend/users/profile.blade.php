			...
			<div class="container">
				<div class="row">
					<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="text-align:center;"> <!-- section title -->
						<h2>{{trans('backend.Your Profile')}}</h2>
					</div> <!-- End section title -->
					<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 form_holder"> <!-- form holder -->
						<form action="{{ action('LoginController@postprofile') }}" class="contact-form" method="POST" role="form">
							<input type="hidden" name="_token" value="{{ csrf_token() }}">
							<input class="form-control name" type="text" name="name" placeholder="{{trans('backend.Your Name') }}" value="{{$user->name}}" Required>
							<input class="form-control email" type="email" name="email" placeholder="{{trans('backend.Your Email') }}" value="{{$user->email}}" Required>
							<input class="form-control phone" type="text" name="phone" placeholder="{{trans('backend.Your phone') }}" value="{{$user->phone}}" Required>
						    ...
						    <button type="submit" class="submit">{{trans('backend.edit') }} <i class="fa fa-arrow-circle-right"></i></button>
