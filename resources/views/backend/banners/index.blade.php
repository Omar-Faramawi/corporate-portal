...
<section class="content-header">
    <h1><i class="livicon" data-name="list" data-size="25" data-c="#418BCA" data-hc="#01BC8C" data-loop="true"></i>{{ trans('backend.list_banners') }}</h1>
    <ol class="breadcrumb">
            <li><a href="/admin"><i class="fa fa-tachometer"></i> {{ trans('backend.dashboard') }}</a></li>
            <li class="active"> {{ trans('backend.list_banners') }}</li>
    </ol>   
</section>
@endsection
        ...
        <div class="portlet box primary">
                ...
                <div class="portlet-body">
                     ...
                     <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        ...
                        @foreach($banners as $item)
                            <tr>
                                ...
                                -->
                                <td class="">
                                <a href="{{action('BannersController@edit', [$item->id])}}"><i class="fa  fa-edit"></i> {{ trans('backend.edit') }}</a>
                        ...
                        <div class="text-center">
                            ...
                            <h1>{{ trans('backend.no_results_found') }}
                             <a href="/{{Lang::getlocale()}}/admin/banners">{{ trans('backend.back') }}</a></h1>
                            @elseif(Request::is(''.Lang::getlocale().'/admin/banners'))
                            <h1>{{ trans('backend.no_data_added_before') }}
                             <a href="/{{Lang::getlocale()}}/admin/banners/create">{{ trans('backend.create_content') }}</a></h1>
                            @endif
                         </div>
                        @endif
                    @if($banners->count())
                   ...
                   <div class="table-footer">
                         <div class="count"> <i class="fa fa-folder-o"></i> {{ trans('backend.total') }} : {{ $banners->total() }} {{ trans('backend.banners') }} </div>
                        <div class="pagination-area"> {!! $banners->render() !!} </div>
                    </div>
                @endif
                </div>
            </div>
...
<script type="text/javascript">
    function bulkconfirmDelete() {
        ...
        http.open("POST", "{{action('BannersController@bulk_destroy_confirm')}}", true);
