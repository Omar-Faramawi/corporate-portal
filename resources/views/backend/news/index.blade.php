...
<section class="content-header">
    <h1><i class="livicon" data-name="list" data-size="25" data-c="#418BCA" data-hc="#01BC8C" data-loop="true"></i>{{ trans('backend.list_news') }}</h1>
    <ol class="breadcrumb">
            <li><a href="/admin"><i class="fa fa-tachometer"></i> {{ trans('backend.dashboard') }}</a></li>
            <li class="active"> {{ trans('backend.list_news') }}</li>
    </ol>   
</section>
@endsection
        ...
        <div class="portlet box primary">
                ...
                <div class="portlet-body">
                     ...
                     <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        ...
                        @foreach($news as $item)
                            <tr>
                                ...
                                <td>
                                    @if($item->published == '1')
                                       <a href="{{action('NewsController@single_status', ['2', $item->id])}}" class="label label-sm label-success"><span class="glyphicon glyphicon-ok"></span> {{ trans('backend.published') }}</a>
                                    @elseif($item->published == '2')
                                        <a href="{{action('NewsController@single_status', ['1', $item->id])}}" class="label label-sm label-warning"><span class="glyphicon glyphicon-ban-circle"></span> {{ trans('backend.unpublished') }}</a>
                                    @endif
                                </td>
                                <td class="">
                                <a href="{{action('NewsController@edit', [$item->id])}}"><i class="fa  fa-edit"></i> {{ trans('backend.edit') }}</a>
                        ...
                        <div class="text-center">
                            ...
                            <h1>{{ trans('backend.no_results_found') }}
                             <a href="/{{Lang::getlocale()}}/admin/news">{{ trans('backend.back') }}</a></h1>
                            @elseif(Request::is(''.Lang::getlocale().'/admin/news'))
                            <h1>{{ trans('backend.no_data_added_before') }}
                             <a href="/{{Lang::getlocale()}}/admin/news/create">{{ trans('backend.create_content') }}</a></h1>
                            @endif
                         </div>
                        @endif
                    @if($news->count())
                   ...
                   <div class="table-footer">
                         <div class="count"> <i class="fa fa-folder-o"></i> {{ trans('backend.total') }} : {{ $news->total() }} {{ trans('backend.news') }} </div>
                        <div class="pagination-area"> {!! $news->render() !!} </div>
                    </div>
                @endif
                </div>
            </div>
...
<script type="text/javascript">
    function bulkconfirmDelete() {
        ...
        http.open("POST", "{{action('NewsController@bulk_destroy_confirm')}}", true);
