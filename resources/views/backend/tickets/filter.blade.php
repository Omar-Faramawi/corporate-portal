          ...
          <div class="box-body">
             <form action="" method="GET" class="filter">
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control" id="" name="title" value="@if (!empty($_GET['title'])){{$_GET['title']}}@endif" placeholder="{{ trans('backend.title') }}">
                    </div>
                    ...
                    <div class="form-group">
                        <label>{{ trans('backend.status') }}</label>
                         <select name="status" class="form-control">
                           ...
                           <option value="1" @if(!empty($_GET['status']) && $_GET['status'] == '1') selected @endif>{{ trans('backend.closed') }}</option>
                           <option value="2" @if(!empty($_GET['status']) && $_GET['status'] == '2') selected @endif>{{ trans('backend.opened') }}</option>
                         </select>
                    </div>
