			...
			<div class="container">
				<div class="row">
					...
					<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="text-align:center;"> <!-- section title -->
						<h2>{{trans('backend.Create New Ticket')}}</h2>
					</div> <!-- End section title -->
					@endif
					<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 form_holder"> <!-- form holder -->
						<form action="{{ action('HomeController@store_create_ticket') }}" class="contact-form" method="POST" role="form">
							<input type="hidden" name="_token" value="{{ csrf_token() }}">
							<input class="form-control title" type="text" name="title" placeholder="{{trans('backend.title') }}" Required>
							<textarea class="form-control description" name="description" placeholder="{{trans('backend.description') }}" Required></textarea>
						    <button type="submit" class="submit">{{trans('backend.submit') }} <i class="fa fa-arrow-circle-right"></i></button>
